...
for sitenum = 1 %loop through sites
    ...
    for pairnum=1%:numel(IM.waveTry)
        ...
        for nseed=1:numel(seedpool)
            ...
            writeSeaState_v412(SeaState);

            % setup AeroDyn input file
            AeroDyn.WakeMod   = 0;
            AeroDyn.InterfaceElevation = designTable.InterfaceElevation(sitenum);
            
            % setting for survival load, numerical stability
            if Vhub>=25
                AeroDyn.AoA34     = 'False'; %setting for survival load
                AeroDyn.UA_Mod    = 0;
            else
                AeroDyn.AoA34     = 'True'; %setting for survival load
                AeroDyn.UA_Mod    = 3;
            end
            writeAerodyn_v412(AeroDyn);
            ...
            ElastoDyn.RotSpeed  = 0.2; % initial rotor speed
            ...
            writeInflowWind_v412(InflowWind);
