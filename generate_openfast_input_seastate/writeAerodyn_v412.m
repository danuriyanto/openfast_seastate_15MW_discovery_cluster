function writeAerodyn_v412(AeroDyn)
...
C(end+1,1) = {[num2str(AeroDyn.WakeMod) '                      Wake_Mod    - Wake/induction model (switch) {0=none, 1=BEMT, 3=OLAF} [Wake_Mod cannot be 2 or 3 when linearizing]']};
...
C(end+1,1) = {['False                  CavitCheck  - Perform cavitation check? (flag) [UA_Mod must be 0 when CavitCheck=true]']};
C(end+1,1) = {['False                  Buoyancy    - Include buoyancy effects? (flag)']};
C(end+1,1) = {['False                  NacelleDrag - Include Nacelle Drag effects? (flag)']};
C(end+1,1) = {['False                  CompAA      - Flag to compute AeroAcoustics calculation [used only when Wake_Mod = 1 or 2]']};
...
C(end+1,1) = {['1                      BEM_Mod     - BEM model {1=legacy NoSweepPitchTwist, 2=polar} (switch) [used for all Wake_Mod to determine output coordinate system]']};
C(end+1,1) = {['--- Skew correction']};
C(end+1,1) = {['1                      Skew_Mod    - Skew model {0=No skew model, -1=Remove non-normal component for linearization, 1=skew model active}']};
C(end+1,1) = {['False                  SkewMomCorr - Turn the skew momentum correction on or off [used only when Skew_Mod=1]']};
C(end+1,1) = {['1                      SkewRedistr_Mod - Type of skewed-wake correction model (switch) {0=no redistribution, 1=Glauert/Pitt/Peters, default=1} [used only when Skew_Mod=1]']};
C(end+1,1) = {['default                SkewRedistrFactor - Constant used in Pitt/Peters skewed wake model {or "default" is 15/32*pi} (-) [used only when Skew_Mod=1 and SkewRedistr_Mod=1]']};
C(end+1,1) = {['--- BEM algorithm']};
C(end+1,1) = {['True                   TipLoss     - Use the Prandtl tip-loss model? (flag) [unused when Wake_Mod=0 or 3]']};
C(end+1,1) = {['True                   HubLoss     - Use the Prandtl hub-loss model? (flag) [unused when Wake_Mod=0 or 3]']};
C(end+1,1) = {['True                   TanInd      - Include tangential induction in BEMT calculations? (flag) [unused when Wake_Mod=0 or 3]']};
C(end+1,1) = {['True                   AIDrag      - Include the drag term in the axial-induction calculation? (flag) [unused when Wake_Mod=0 or 3]']};
C(end+1,1) = {['True                   TIDrag      - Include the drag term in the tangential-induction calculation? (flag) [unused when Wake_Mod=0,3 or TanInd=FALSE]']};
C(end+1,1) = {['Default                IndToler    - Convergence tolerance for BEMT nonlinear solve residual equation {or "default"} (-) [unused when Wake_Mod=0 or 3]']};
C(end+1,1) = {['500                    MaxIter     - Maximum number of iteration steps (-) [unused when Wake_Mod=0]']};
C(end+1,1) = {['--- Shear correction']};
C(end+1,1) = {['False                  SectAvg     - Use sector averaging (flag)']};
C(end+1,1) = {['1                      SectAvgWeighting - Weighting function for sector average {1=Uniform, default=1} within a sector centered on the blade (switch) [used only when SectAvg=True]']};
C(end+1,1) = {['default                SectAvgNPoints - Number of points per sectors (-) {default=5} [used only when SectAvg=True]']};
C(end+1,1) = {['default                SectAvgPsiBwd - Backward azimuth relative to blade where the sector starts (<=0) {default=-60} (deg) [used only when SectAvg=True]']};
C(end+1,1) = {['default                SectAvgPsiFwd - Forward azimuth relative to blade where the sector ends (>=0) {default=60} (deg) [used only when SectAvg=True]']};
C(end+1,1) = {['--- Dynamic wake/inflow']};
C(end+1,1) = {['2                      DBEMT_Mod   - Type of dynamic BEMT (DBEMT) model {0=No Dynamic Wake, -1=Frozen Wake for linearization, 1:constant tau1, 2=time-dependent tau1, 3=constant tau1 with continuous formulation} (-)']};
C(end+1,1) = {['29.03                  tau1_const  - Time constant for DBEMT (s) [used only when Wake_Mod=2 and DBEMT_Mod=1]']};
C(end+1,1) = {['======  OLAF -- cOnvecting LAgrangian Filaments (Free Vortex Wake) Theory Options  ================== [used only when Wake_Mod=3]']};
C(end+1,1) = {['../IEA-15-240-RWT-OLAF/IEA-15-240-RWT_OLAF.dat OLAFInputFileName - Input file for OLAF [used only when Wake_Mod=3]']};
C(end+1,1) = {['======  Unsteady Airfoil Aerodynamics Options  ====================================================']};
C(end+1,1) = {[AeroDyn.AoA34 '                   AoA34       - Sample the angle of attack (AoA) at the 3/4 chord or the AC point {default=True} [always used]']};
C(end+1,1) = {[num2str(AeroDyn.UA_Mod) '                      UA_Mod      - Unsteady Aero Model Switch (switch) {0=Quasi-steady (no UA), 2=B-L Gonzalez, 3=B-L Minnema/Pierce, 4=B-L HGM 4-states, 5=B-L HGM+vortex 5 states, 6=Oye, 7=Boeing-Vertol}']};
C(end+1,1) = {['True                   FLookup     - Flag to indicate whether a lookup for fprime will be calculated (TRUE) or whether best-fit exponential equations will be used (FALSE); if FALSE S1-S4 must be provided in airfoil input files (flag) [used only when UA_Mod>0]']};
C(end+1,1) = {['3               IntegrationMethod  - Switch to indicate which integration method UA uses (1=RK4, 2=AB4, 3=ABM4, 4=BDF2)']};
C(end+1,1) = {['0.1                      UAStartRad  - Starting radius for dynamic stall (fraction of rotor radius [0.0,1.0]) [used only when UA_Mod>0; if line is missing UAStartRad=0]']};
C(end+1,1) = {['1                      UAEndRad    - Ending radius for dynamic stall (fraction of rotor radius [0.0,1.0]) [used only when UA_Mod>0; if line is missing UAEndRad=1]']};
...
C(end+1,1) = {['======  Nacelle Properties ========================================================================== [used only when Buoyancy=True or NacelleDrag=True]']};
C(end+1,1) = {['0                      VolNac      - Nacelle volume (m^3)']};
C(end+1,1) = {['0.0, 0.0, 0.0   NacCenB            - Position of nacelle center of buoyancy from yaw bearing in nacelle coordinates (m)']};
C(end+1,1) = {['0, 0, 0      NacArea        - Projected area of the nacelle in X, Y, Z in the nacelle coordinate system (m^2)']};
C(end+1,1) = {['0, 0, 0   NacCd          - Drag coefficient for the nacelle areas defined above (-)']};
C(end+1,1) = {['0, 0, 0         NacDragAC          - Position of aerodynamic center of nacelle drag in nacelle coordinates (m)']};
...
C(end+1,1) = {['(m)             (m)             (-)             (-)             (-)']};
C(end+1,1) = {[num2str(AeroDyn.InterfaceElevation) '              10              5.00E-01        1.00E-01        0']};
C(end+1,1) = {['28              10              5.00E-01        1.00E-01        0']};
C(end+1,1) = {['41              9.93E+00        5.00E-01        1.00E-01        0']};
C(end+1,1) = {['67              8.83E+00        5.00E-01        1.00E-01        0']};
C(end+1,1) = {['80              8.15E+00        5.00E-01        1.00E-01        0']};
C(end+1,1) = {['93              7.39E+00        5.00E-01        1.00E-01        0']};
C(end+1,1) = {['106             6.91E+00        5.00E-01        1.00E-01        0']};
C(end+1,1) = {['119             6.75E+00        5.00E-01        1.00E-01        0']};
C(end+1,1) = {['132             6.57E+00        5.00E-01        1.00E-01        0']};
C(end+1,1) = {['1.44E+02        6.50E+00        5.00E-01        1.00E-01        0']};
C(end+1,1) = {['======  Outputs  ====================================================================================']};
...
C(end+1,1) = {['   1              BldNd_BladesOut     - Number of blades to output all node information at.  Up to number of blades on turbine. (-)']};
