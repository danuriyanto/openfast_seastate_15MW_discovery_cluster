function writeElastoDyn_seastate(ElastoDyn)
...
C(end+1,1) = {[num2str(ElastoDyn.BlPitch,1)           '             BlPitch(1)    - Blade 1 initial pitch (degrees)']};
C(end+1,1) = {[num2str(ElastoDyn.BlPitch,1)           '             BlPitch(2)    - Blade 2 initial pitch (degrees)']};
C(end+1,1) = {[num2str(ElastoDyn.BlPitch,1)           '             BlPitch(3)    - Blade 3 initial pitch (degrees) [unused for 2 blades]']};
C(end+1,1) = {['0.0           TeetDefl      - Initial or fixed teeter angle (degrees) [unused for 3 blades]']};
C(end+1,1) = {[num2str(ElastoDyn.Azimuth,1)         '           Azimuth       - Initial azimuth angle for blade 1 (degrees)']};
C(end+1,1) = {[num2str(ElastoDyn.RotSpeed,2)     '            RotSpeed      - Initial or fixed rotor speed (rpm)']};
C(end+1,1) = {[num2str(ElastoDyn.NacYaw,1)        '             NacYaw        - Initial or fixed nacelle-yaw angle (degrees)']};
...
C(end+1,1) = {['20.0          TowerBsHt     - Height of tower base above ground level [onshore] or MSL [offshore] (meters)']};
...
C(end+1,1) = {['20.0          PtfmCMzt      - Vertical distance from the ground level [onshore] or MSL [offshore] to the platform CM (meters)']};
C(end+1,1) = {['20.0          PtfmRefzt     - Vertical distance from the ground level [onshore] or MSL [offshore] to the platform reference point (meters)']};
...
C(end+1,1) = {['"YawBrTDxp, YawBrTDyp, YawBrTDzp"']};
C(end+1,1) = {['"YawBrFxp, YawBrFyp, YawBrFzp, YawBrMxn, YawBrMyn, YawBrMzn, NacYaw, NacYawV,NacYawA"']};
C(end+1,1) = {['END	']};
